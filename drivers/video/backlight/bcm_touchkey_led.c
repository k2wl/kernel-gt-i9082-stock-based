...
static void bcm_touchkey_bln_enable(void)
{
	keyled_power_ctrl(1); // KEYLED_ON
}

static void bcm_touchkey_bln_disable(void)
{
	keyled_power_ctrl(0); // KEYLED_OFF
}

static struct bln_implementation bcm_touchkey_bln = {
	.enable = bcm_touchkey_bln_enable,
	.disable = bcm_touchkey_bln_disable,
};
#endif
...
#ifdef CONFIG_GENERIC_BLN
	register_bln_implementation(&bcm_touchkey_bln);
#endif
